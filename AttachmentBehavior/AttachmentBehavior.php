...
class AttachmentBehavior extends CActiveRecordBehavior {
	...
	{
		$file = new AttachmentUploadedFile($this->Owner,$this->attribute . '_attachment');

		if($file->isFileExists){ // something was uploaded
		...
		elseif ($file->deleteCurrent) { //nothing uploaded, but setted flag we want to delete current attachment
			$this->deleteAttachment();
			$this->Owner->saveAttributes(array($this->attribute => null));
		}
		return true;
	}
...
class AttachmentUploadedFile
{
	...
	public function __construct($model, $index) {
		...
		$this->name = $_FILES[$modelName]['name'][$index];
		$this->tmpName = $_FILES[$modelName]['tmp_name'][$index];
		$this->deleteCurrent = isset($_POST[$modelName][$index . '_delete']) && $_POST[$modelName][$index . '_delete'];
		$this->isFileExists = file_exists($this->tmpName);
	}

	public function saveAs($file){
		if($this->isFileExists && $this->error==UPLOAD_ERR_OK){
			return rename($this->tmpName, $file);
		}else return false;
	}
}
